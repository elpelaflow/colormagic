# Worker color-renderer — Informe técnico (Fases 2a + 2b)
...
| `pool.mjs` | Pool de browser: launch-once + `BrowserContext` aislado por job + semáforo con cola |
| `render.mjs` | Job: navega → settle 2s → muestreo de computed styles + texto/fondo efectivo para contraste |
| `lib.mjs` | Funciones puras (testeables sin Playwright): `rgbToHex`, `aggregateUsage`, `createLruCache`, `contrastRatio`, `aggregateContrast` |
| `Dockerfile` | Base oficial `mcr.microsoft.com/playwright:v1.62.1-jammy` |

## 3. Contrato API

### `POST /render` — body `{ url, screenshot?: boolean }`
...
|---|---|---|
| `usagePalette` | array | Colores renderizados con count y share (%) — top 40 |
| `contrast` | array | Pares fg/bg con ratio (1-21) y pases AA/AAA — top 20 |
...
- Detección de dark mode (dos renders: light + dark) y merge de tokens declarados vs. computados.
