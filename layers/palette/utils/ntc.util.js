...
import colorNames from './color-names-data.json';
...
const ntc = {
  ...
  names: colorNames,
  uniqueNames: function (hexes) {
    const seen = {};
    return hexes.map(hex => {
      const base = this.name(hex)[1].toString();
      if (seen[base] === undefined) { seen[base] = 1; return base; }
      seen[base] += 1;
